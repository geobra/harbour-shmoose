...
XmlWriter::XmlWriter( )
    : out(new QString), indentSize( 4 ), autoNewLine( false ), atBeginningOfLine( true )
{
    out.setCodec("UTF-8");
}
...
void XmlWriter::writeTaggedString( const QString& name, const QString& string, const AttrMap& attrs )
{
    writePendingIndent();
    out << opening( name, attrs );
    writeString( string );
...
const QString XmlWriter::getXmlResult()
{
    out.flush();
    return out.readAll();
}
